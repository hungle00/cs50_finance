...
def history():
    ...
    transactions = db.execute(
        "SELECT symbol, shares, price, transacted FROM transactions WHERE user_id = :user_id ORDER BY transacted ASC", user_id=session["user_id"])
...
def register():
    ...
    if request.method == "POST":
        ...
        if not request.form.get("username"):
            ...
            return apology("must provided password", 400)
        elif not request.form.get("password") == request.form.get("confirmation"):
            return apology("pass don't match", 400)
